...
let length =
  let doc = "Number of bytes to trim in each execution." in
  Arg.(value & opt int 64000000 & info [ "l"; "length" ] ~docv:"LENGTH" ~doc)

let max_queue =
  let doc =
    "Max queue size allowed before starting to sleep between executions."
  in
  Arg.(value & opt float 1. & info [ "m"; "max-queue" ] ~docv:"MAX_QUEUE" ~doc)

let info =
  let doc = "Run fstrim but with rudimentary throttling." in
  Term.info "throttle-fstrim" ~version:"%%VERSION%%" ~doc
...
let throttle_fstrim_t = Term.(const main $ length $ max_queue)

let () = Term.exit @@ Term.eval (throttle_fstrim_t, info)
